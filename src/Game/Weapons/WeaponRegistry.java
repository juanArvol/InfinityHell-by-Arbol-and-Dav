...
public final class WeaponRegistry {
     ...
     */
    public static List<Game.Bullets.BulletType> buildBulletOfferPool(
            Set<Game.Bullets.BulletType> alreadyOwned, int maxCount, Random random) {

        List<Game.Bullets.BulletType> candidates = new ArrayList<>();
        for (Game.Bullets.BulletType bt : Game.Bullets.BulletType.values()) {
        ...
        List<Game.Bullets.BulletType> result = new ArrayList<>();
        Set<Game.Bullets.BulletType> selected = new HashSet<>();
        ...
        while (result.size() < maxCount && result.size() < candidates.size() && attempts < 100) {
            ...
            int roll = random.nextInt(totalWeight);
            int acc  = 0;
            for (Game.Bullets.BulletType bt : candidates) {
