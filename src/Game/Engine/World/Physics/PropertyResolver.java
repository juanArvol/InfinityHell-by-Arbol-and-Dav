 ...
 * ── FLUJO ─────────────────────────────────────────────────────────────────
 *
 *   1. El LawResolver o PhysicsCoordinator notifica que una propiedad cambió:
 ...
 *      LawResolver debe evaluar, respetando prioridades y distinguiendo
 *      propiedades cíclicas (que requieren iteración convergente).
 *
 *   4. El LawResolver ejecuta las leyes en el orden indicado por el plan.
 *
...
public final class PropertyResolver {
     ...
     */
    public PropertyResolver(PropertyDependencyGraph graph) {
        if (graph == null)
     ...
     * el LawResolver debe recalcular como consecuencia de los cambios
     * marcados mediante {@link #markChanged}.
     *
     * Las propiedades cíclicas aparecen en el plan con su flag de ciclo
     * activado; el LawResolver las evalúa de forma iterativa.
     *
    ...
    public ResolutionPlan resolve() {
        ...
        List<PropertyDependencyGraph.PropagationStep> steps    = new ArrayList<>();
        List<PhysicalProperty>                        cyclics  = new ArrayList<>();

        for (PhysicalProperty changed : changedProperties) {
            List<PropertyDependencyGraph.PropagationStep> propagation =
                graph.propagationOrderFrom(changed);

            for (PropertyDependencyGraph.PropagationStep step : propagation) {
     ...
     *             El LawResolver ejecuta cada una una sola vez, en secuencia.
     *
     *   cyclics → propiedades que forman parte de ciclos físicos.
     *             El LawResolver las evalúa iterativamente hasta convergencia
     *             (máximo MAX_CYCLE_ITERATIONS iteraciones).
     *
     * El LawResolver consume este plan. El PropertyResolver no lo ejecuta.
     */
    public static final class ResolutionPlan {

        /** Pasos de propagación en orden topológico. Sin ciclos. */
        private final List<PropertyDependencyGraph.PropagationStep> steps;
        ...
        ResolutionPlan(List<PropertyDependencyGraph.PropagationStep> steps,
                       List<PhysicalProperty>                        cyclics) {
            this.steps   = steps;
            this.cyclics = cyclics;
        }

        /** Plan vacío. El LawResolver no tiene nada que hacer. */
         ...
         */
        public List<PropertyDependencyGraph.PropagationStep> getSteps() {
            return steps;
        }

        /**
         * Propiedades participantes en ciclos físicos.
         * El LawResolver las evalúa por iteración convergente.
         *
