...
/**
 * Coordinador del Physics Core. Orquesta PropertyResolver y LawResolver.
 *
 ...
 *   [2] LawResolver      — evalúa las relaciones físicas indicadas por el
...
public final class PhysicsCoordinator {
    ...
    private final LawResolver lawResolver;
    ...
    private PhysicsCoordinator(Builder b) {
        ...
        this.lawResolver      = new LawResolver(b.registry);
    }
    ...
    public void simulate(List<GameObjects> objects, double deltaTime) {
        ...
        if (lawResolver.isEmpty()) return;

        if (firstFrame || graph.isEmpty()) {
            lawResolver.evaluateAll(objects, deltaTime);
            firstFrame = false;
        } else {
    ...
    /**
     * El LawResolver de este coordinator.
     *
     * Usar para registrar relaciones adicionales en runtime:
     *   coordinator.lawResolver().register(gravityRelation)
     *
     * @return el LawResolver de este coordinator.
     */
    public LawResolver lawResolver() {
        return lawResolver;
    }

    /**
     * Registra una PhysicalRelation directamente en el LawResolver.
    ...
    public PhysicsCoordinator registerAll(RelationRegistry registry) {
        lawResolver.registerAll(registry);
        return this;
    }
