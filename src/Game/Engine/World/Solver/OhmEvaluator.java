...
public final class OhmEvaluator implements RelationEvaluator {
    ...
    public void evaluate(PhysicalRelation     relation,
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            if (!a.has(CoreProperties.CHARGE)) continue;

            for (int j = i + 1; j < n; j++) {
                EvaluationView b = views.get(j);
                if (!b.has(CoreProperties.CHARGE)) continue;

                if (distance(a, b) > maxDist) continue;

                double qA   = a.get(CoreProperties.CHARGE);
                double qB   = b.get(CoreProperties.CHARGE);
                double diff = qA - qB;
                if (Math.abs(diff) < minDelta) continue;

                double sA = a.has(CoreProperties.ELECTRICAL_CONDUCTIVITY)
                    ? a.get(CoreProperties.ELECTRICAL_CONDUCTIVITY)
                    : DEFAULT_CONDUCTIVITY;
                double sB = b.has(CoreProperties.ELECTRICAL_CONDUCTIVITY)
                    ? b.get(CoreProperties.ELECTRICAL_CONDUCTIVITY)
                    : DEFAULT_CONDUCTIVITY;
                ...
                a.add(CoreProperties.CHARGE, -transferred);
                b.add(CoreProperties.CHARGE,  transferred);
