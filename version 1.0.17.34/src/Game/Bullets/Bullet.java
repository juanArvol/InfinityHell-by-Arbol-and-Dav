...
public class Bullet extends GameObjects {
    ...
    public Bullet(Vector2D position, BufferedImage texture, double x, double y, bulletType tipo, Player p, ArrayList<EnimyNormal> enemies) {
    ...
    private boolean checkCollision() {
        for (GameObjects obj : GameState.instance.getObjects()) { // acceso al GameState
            if (obj != this && getBounds().intersects(obj.getBounds())) {
                // simulamos lo que ya hace onCollision
                onCollision(obj);
                return true; // detenemos movimiento de la bala
            }
        }
        return false;
    }
